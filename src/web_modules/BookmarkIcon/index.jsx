...
export default class BookmarkIcon extends Component {
     ...
     */
    render() {
        let { show, active, reverseColor } = this.props;
        if(reverseColor === undefined) {
            reverseColor = false;
        }
        else {
            console.log(show);
        ...
        return (
            <svg width="15" height="20" viewBox="0 0 15 20" xmlns="http://www.w3.org/2000/svg"
                ...
                <path className={ ctx({
                    [styles.simpleIcon]: (!reverseColor && !active) || (reverseColor && active),
                    [styles.strongIcon]: (!reverseColor && active) || (reverseColor && !active)
